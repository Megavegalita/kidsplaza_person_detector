...
class ResNet50GenderClassifier:
    ...
    def __init__(
        ...
        if model_path is None:
            model_path = Path(__file__).parent.parent.parent.parent / 'models' / 'resnet50_gender_pytorch.pth'
    ...
    def _load_model(self) -> nn.Module:
        ...
        if self.model_path.exists():
            try:
                ...
                logger.warning(f"Could not load custom weights: {e}, using ImageNet weights")
        else:
    ...
    def classify(self, face_crop: np.ndarray) -> Tuple[str, float]:
        ...
        try:
            ...
            crop_tensor = torch.from_numpy(crop_normalized).permute(2, 0, 1).unsqueeze(0).float()
            ...
            with torch.no_grad():
                ...
                logger.debug(f"ResNet50 gender: {predicted_gender} (conf={confidence:.3f}, M={male_prob:.3f}, F={female_prob:.3f})")
