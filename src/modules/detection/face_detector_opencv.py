...
class FaceDetectorOpenCV:
    ...
    def __init__(
        ...
        detect_resize: Optional[
            tuple[int, int]
        ] = None,  # Resize frame for faster detection
    ) -> None:
        """
        ...
        try:
            ...
            if faces is not None and len(faces) > 0:
                ...
                for face in filtered_faces:
                    ...
                    logger.debug(
                        "Face confidence: %.3f, threshold: %.3f",
                        confidence,
                    ...
                    if channel_id in [3, 4]:
                        min_face_size = (
                            16  # Indoor: faces close but may be small after scaling
                        )
                    else:
                        min_face_size = (
                            12  # Outdoor: faces far away, accept smaller after scaling
                        )
    ...
    def _validate_face_landmarks(
        ...
        if valid_landmarks < 3:
            logger.debug(
                "Landmark validation failed: too few valid landmarks (%d < 3)",
                valid_landmarks,
            )
            return False
        ...
        for x, y in landmarks:
            ...
            if (
                x < face_x1 - face_w * tolerance
                or x > face_x2 + face_w * tolerance
                or y < face_y1 - face_h * tolerance
                or y > face_y2 + face_h * tolerance
            ):
        ...
        if eye_height_diff > face_h * 0.3:
            logger.debug(
                "Landmark validation failed: eyes not aligned (diff=%.1f > %.1f)",
                eye_height_diff,
                face_h * 0.3,
            )
        ...
        if nose_horizontal_offset > face_w * 0.4:
            logger.debug(
                "Landmark validation failed: nose too far from center (offset=%.1f > %.1f)",
        ...
        if not (eye_center_y < nose_tip_y < mouth_center_y):
            logger.debug(
                "Landmark validation failed: invalid vertical ordering (eyes=%.1f, nose=%.1f, mouth=%.1f)",
                eye_center_y,
                nose_tip_y,
                mouth_center_y,
            )
        ...
        if eye_symmetry_diff > face_w * 0.35:
            logger.debug(
                "Landmark validation failed: eyes too asymmetric (diff=%.1f > %.1f)",
                eye_symmetry_diff,
                face_w * 0.35,
            )
