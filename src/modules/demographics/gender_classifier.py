...
class GenderClassifier:
    ...
    def _build_model(self, model_type: str) -> nn.Module:
        ...
        if model_type == 'timm_mobile' and TIMM_AVAILABLE:
            logger.info("Using MobileNetV3-Small from timm for gender classification (RECOMMENDED)")
            ...
            logger.warning(f"Unsupported model_type: {model_type}, falling back to simple CNN")
            model = nn.Sequential(
            # Input: 3x64x64
    ...
    def classify(
        ...
        try:
            ...
            logger.debug(f"Classifying gender for track_id={track_id}, crop shape={crop.shape}")
            ...
            with torch.no_grad():
                ...
                logger.debug(f"Track_id={track_id}: Raw probs class0={class_0_prob:.3f} class1={class_1_prob:.3f}")
            ...
            logger.debug(f"Track_id={track_id}: Mapped prediction: class={predicted_class} -> gender={gender}, conf={confidence_val:.3f}")
            
            # Apply minimum confidence threshold
            if confidence_val < self.min_confidence:
                logger.debug(f"Track_id={track_id}: Low confidence {confidence_val:.2f} < {self.min_confidence}, returning 'Unknown'")
            ...
            if track_id is not None:
                ...
                if len(self._prediction_history[track_id]) >= 3:
                    recent_genders = [p['gender'] for p in self._prediction_history[track_id]]
                    gender_counts = {}
                    ...
                    last_confidence = self._prediction_history[track_id][-1]['confidence']
                    if last_confidence > 0.7:
                        logger.debug(f"Track_id={track_id}: Majority vote={vote_gender} (history: {recent_genders}), confidence={last_confidence:.2f}")
                        return vote_gender, last_confidence
            
            logger.debug(f"Track_id={track_id}: Direct prediction={gender}, confidence={confidence_val:.2f}")
    ...
    def get_stable_prediction(self, track_id: int) -> Optional[Tuple[str, float]]:
        ...
        vote_gender = max(gender_counts.items(), key=lambda x: x[1])[0]
        avg_confidence = np.mean([p['confidence'] for p in history[-self.voting_window:]])
