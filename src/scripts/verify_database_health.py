...
def check_database_dependencies() -> Dict[str, bool]:
    ...
    try:
        ...
        print("Warning: psycopg2 not installed. PostgreSQL health check will be skipped.")
...
def verify_postgresql_connection(
    ...
    try:
        ...
        return True, f"PostgreSQL connected successfully - Version: {version[0][:50]}", response_time
...
def verify_redis_connection(
    ...
    try:
        ...
        return True, f"Redis connected successfully - Version: {version}, Memory: {used_memory}", response_time
...
def verify_postgresql(
    ...
    start_time = time.time()
    is_healthy, status_message, response_time = verify_postgresql_connection(config, timeout)
...
def verify_all_databases(config_path: str, timeout: int = 5) -> Dict:
    ...
    return {
        ...
        'health_rate': f"{(healthy_count / len(results) * 100):.1f}%" if results else "N/A",
        'timestamp': datetime.now().isoformat(),
        'databases': results
    }
...
def main():
    ...
    default_config_path = Path(__file__).parent.parent.parent / "config" / "database.json"
