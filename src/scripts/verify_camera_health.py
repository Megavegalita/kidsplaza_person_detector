...
def generate_alternative_urls(
    server: Dict, credentials: Dict, channel_id: int
) -> List[str]:
    """
...
def verify_rtsp_connection(rtsp_url: str, timeout: int = 10) -> Tuple[bool, str]:
    ...
    try:
        ...
        return (
            True,
            f"Stream active - Frame: {width}x{height}, Latency: {elapsed_time:.2f}s",
        )
...
def verify_camera_channel(channel: Dict, timeout: int = 10) -> Dict:
    """
