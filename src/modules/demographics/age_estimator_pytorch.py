        ...
        try:
            if model_name.startswith("huggingface") and TRANSFORMERS_AVAILABLE:
                ...
                for hf_model_name, model_type, is_8_class in hf_models:
                    try:
                        logger.info("Attempting to load Hugging Face model: %s (%s)", hf_model_name, model_type)
                        ...
                        try:
                            ...
                            logger.debug("Could not load processor: %s, using manual preprocessing", proc_e)
                            self.processor = None
                        ...
                        try:
                            ...
                            logger.debug("Failed with trust_remote_code=True: %s, trying without", model_e1)
                            try:
                        ...
                        logger.debug("Failed to load %s: %s, trying next...", hf_model_name, e)
                        continue
                
                # All Hugging Face models failed
                logger.warning("All Hugging Face models failed to load, using torchvision fallback")
    ...
    def _load_torchvision_model(self, model_name: str) -> None:
        ...
        try:
            if model_name == "torchvision_resnet18":
                ...
                logger.info("Loaded ResNet18 for age estimation (ImageNet pretrained, needs fine-tuning)")
            ...
            model_path = Path(__file__).parent.parent.parent.parent / "models" / "age_resnet18_pytorch.pth"
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if hasattr(outputs, 'logits'):
                    ...
                    logger.debug("Age model using Hugging Face format, logits shape: %s", logits.shape)
                    if logits.dim() > 1:
                        ...
                        if self._is_8_class_model and num_classes == 8:
                            ...
                            if predicted_class < len(age_ranges_8class):
                                ...
                                age = int((age_min + age_max) / 2)  # Use middle of range
                                logger.debug(
                                    "8-class model: class %d -> age range %d-%d, using %d",
                                    predicted_class, age_min, age_max, age
                                )
                            else:
                    ...
                    age = float(outputs[0]) if isinstance(outputs, (list, tuple)) else 0.0
                    confidence = 0.7
                ...
                if abs(original_age - age) > 2:
                    logger.debug(
                        ...
                        original_age, age, self._hf_model_name or self.model_name
                    )
